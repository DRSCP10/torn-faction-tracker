...
const DEFAULT_SETTINGS = () => ({
  ...
  public: {
    ...
    /** Unix seconds (TCT/UTC) when current war/ranked window started; null if unset */
    warStartedAt: null,
    warLabel: 'War',
    /** Hours into day (or since war start) before showing "no hits yet" list */
...
export function getWarConfig(settings) {
  const pub = settings?.public || {};
  const warStartedAt = pub.warStartedAt
    ? Math.floor(Number(pub.warStartedAt))
    : null;
  return {
    warStartedAt: warStartedAt > 0 ? warStartedAt : null,
    warLabel: (pub.warLabel || 'War').trim() || 'War',
    inactiveAfterHours: Math.max(
      0,
      Number(pub.inactiveAfterHours ?? 2) || 2
    ),
  };
}

export async function loadWarConfig() {
  const settings = await loadSettings();
  return getWarConfig(settings);
}
