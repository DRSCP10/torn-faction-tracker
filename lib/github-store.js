...
import { isAdminBackedPath, storageKeyForPath } from './admin-storage.js';
import {
...
export async function loadDataFile(filePath, defaultValue) {
  const storageKey = storageKeyForPath(filePath);

  if (isPostgresConfigured() && storageKey) {
    const fromPg = await pgGetJson(storageKey);
    if (fromPg !== null) return fromPg;
  }

  const kvKey = kvKeyForPath(filePath);
  if (isKvConfigured() && kvKey) {
    const fromKv = await kvGetJson(kvKey);
...
export async function saveDataFile(filePath, data, commitMessage) {
  ...
  if (isPostgresConfigured() && storageKey) {
    await pgSetJson(storageKey, data);
    return { method: 'postgres' };
  }

  const kvKey = kvKeyForPath(filePath);
  if (isKvConfigured() && kvKey) {
    await kvSetJson(kvKey, data);
    return { method: 'kv' };
  }
  ...
  if (!token) {
    if (isAdminBackedPath(filePath)) {
      throw new Error(
        'Cannot save: connect Vercel Postgres (POSTGRES_URL), Redis/KV, or set GITHUB_ACCESS_TOKEN'
      );
    }
    throw new Error(
